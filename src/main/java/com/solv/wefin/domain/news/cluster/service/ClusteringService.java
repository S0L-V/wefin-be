...
public class ClusteringService {
    ...
    public void clusterPendingArticles() {
        ...
        log.info("클러스터링 완료 - 매칭: {}, 신규: {}, scoring거부→신규: {}, suspicious: {}, 총 클러스터: {}, 평균 size: {}",
                matchedCount, createdCount, createdByRejectionCount, suspiciousCount,
    ...
    private ClusterResult processArticle(NewsArticle article, List<NewsCluster> activeClusters) {
        ...
        if (articleVector == null) {
            log.warn("임베딩이 없는 기사 → 신규 클러스터 생성 불가 - articleId: {}", article.getId());
    ...
    private enum ClusterResult {
        MATCHED, SUSPICIOUS_MATCHED, CREATED, CREATED_BY_REJECTION
    }
