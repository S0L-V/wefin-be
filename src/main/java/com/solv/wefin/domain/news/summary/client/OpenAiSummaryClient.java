...
public class OpenAiSummaryClient {
    ...
    private static final String SINGLE_ARTICLE_SUMMARY_PROMPT = """
            당신은 금융 뉴스 전문 에디터입니다.
            기사 한 건의 본문을 읽고 핵심 내용을 구조화된 브리핑으로 작성하세요.
            ...
            3. sections: 상세 분석 섹션 배열 (2~3개)
               - 기사 한 건이라도 관점별로 나누어 분석할 것
               ...
               - body: 해당 논점에 대한 상세 설명 (400~600자, 5~7문장)
                 · 구체적 수치, 기업명, 날짜를 포함할 것
                 · "신중 필요", "영향 가능" 같은 일반적 표현 대신 구체적 근거를 제시
               - sourceArticleIndices: [1] (기사가 1건이므로 항상 [1])
            4. suggestedQuestions: 추천 질문 배열 (정확히 3개)
               - 기사를 읽은 사용자가 추가로 궁금해할 만한 질문
            ...
            반드시 아래 JSON 형식으로만 응답하세요:
            {
              ...
              "sections": [
                {"heading": "소제목", "body": "상세 설명...", "sourceArticleIndices": [1]}
              ],
