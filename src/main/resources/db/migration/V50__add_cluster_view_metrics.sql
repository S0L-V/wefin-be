...
-- IF NOT EXISTS 로 멱등성 확보 — 이전 partial 실행으로 컬럼만 남은 로컬 환경 대응
ALTER TABLE news_cluster
    ADD COLUMN IF NOT EXISTS unique_viewer_count BIGINT NOT NULL DEFAULT 0,
    ADD COLUMN IF NOT EXISTS recent_view_count   BIGINT NOT NULL DEFAULT 0;
...
CREATE INDEX IF NOT EXISTS idx_news_cluster_hot
...
CREATE INDEX IF NOT EXISTS idx_user_news_cluster_read_at
...
CREATE TABLE IF NOT EXISTS hot_aggregation_meta (
